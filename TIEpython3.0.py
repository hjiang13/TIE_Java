...
guiPlus.addNumericField("dz: ", default_dz, 2)
guiPlus.addNumericField("Lambda: ", default_Lambda, 3)
guiPlus.addNumericField("Global offset:", default_globaloffset, 2)
guiPlus.addNumericField("w0:", default_w0, 2)
guiPlus.addNumericField("n: ", default_n, 2)
'''
...
if guiPlus.wasOKed():
    ...
    if doOption == True:
        BG_array = array.array('f')
        BG_width = Ia.getProcessor().getWidth()
        BG_height = Ia.getProcessor().getHeight()
        BG_size = BG_width * BG_height
        for i in range(BG_size):
            BG_array.append(0)
        BG_matrix = split_list(BG_array, wanted_parts = BG_width)
        BG = ImagePlus("BG", FloatProcessor(BG_matrix))
        BG.show()
    else:
        BG = guiPlus.getNextImage()
...
hanning = array.array("f")
for i in range(len_wY):
    for j in range(len_wX):
        #hanning[i * len_wX + j] = globaloffset * (1 + math.cos(hanning[i * len_wX + j] * pi / w0))
        hanning.append(globaloffset * (1 + math.cos(wR[i * len_wX + j] * pi / w0)))
...
#IJ.run(DI, "FFT", "")
IJ.run("FFT Options...", "fft complex do");
...
TIEH = array.array("f")
for i in range(len(wR)):
    TIEH.append(- 1/ ((4 * pi**2)*(ww[i]**2 + hanning[i])))
print(TIEH)

TIEGO = array.array("f")
for i in range(len(wR)):
    TIEGO.append(- 1/ ((4 * pi**2)*(ww[i]**2 + globaloffset)))
...
IJ.setSlice(2)
IJ.run("Delete Slice")
IJ.makeRectangle(0, 0, Dimx, Dimy)
IJ.run("Duplicate...", " ")
#IJ.run("Rotate 90 Degrees Left")
#IJ.run("Flip Vertically")
...
pixel_matrix01 = split_list(phiH, wanted_parts = Dimy)
TIE = ImagePlus("TIE", FloatProcessor(pixel_matrix01))
TIE.show()
IJ.run("Rotate 90 Degrees Left")
IJ.run("Flip Vertically")

pixel_matrix02 = split_list(RI, wanted_parts = Dimy)
...
for i in range(0, Dimy):
    for j in range(0, Dimx - 1):
        file1.write(str(phiH[i * Dimx + j]) + ",")
    file1.write(str(phiH[i * Dimx + Dimx - 1]) + "\n")
file1.close()

file2 = open(directory + "/RI_.csv", "w")
for i in range(0, Dimy):
    for j in range(0, Dimx - 1):
        file2.write(str(RI[i * Dimx + j]) + ",")
    file2.write(str(RI[i * Dimx + Dimx - 1]) + "\n")
file2.close()
